...
@section('page-title', 'Low Stock Alert Items')
...
<div class="space-y-6">
    ...
    <!-- Alert -->
    <div class="card bg-yellow-50 border-l-4 border-yellow-500 p-6 rounded-lg">
        <h3 class="text-lg font-semibold text-yellow-900 mb-2">
            <i class="fas fa-exclamation-triangle"></i> Stock Alert
        </h3>
        <p class="text-yellow-800">
            These items have fallen below their minimum stock threshold. Please restock them soon to prevent shortages.
        </p>
    </div>
    ...
    <div class="card bg-white rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">
                Low Stock Items ({{ $inventories->count() }})
            </h3>
        </div>
            ...
            <table class="w-full table-hover">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Min. Required</th>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Estimated Cost to Restock</th>
                    ...
                    @forelse($inventories as $inventory)
                        <tr class="hover:bg-yellow-50">
                            ...
                            <td class="px-6 py-4 text-sm font-semibold text-gray-800">
                                ₱{{ number_format(($inventory->reorder_level - $inventory->quantity) * $inventory->unit_price, 2) }}
                            </td>
                            <td class="px-6 py-4 text-right text-sm space-x-2">
                                <a href="{{ route('inventories.show', $inventory) }}" class="text-blue-600 hover:text-blue-800" title="View">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <button type="button" onclick="restockItem({{ $inventory->id }}, '{{ $inventory->name }}')" 
                                        class="text-green-600 hover:text-green-800" title="Restock">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p>No low stock items. Everything is well stocked!</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="grid grid-cols-3 gap-6">
        <div class="card bg-white p-6 rounded-lg">
            ...
            <div class="space-y-3">
                <div>
                    <p class="text-xs text-gray-500 uppercase">Items in Alert</p>
        ...
        <div class="card bg-white p-6 rounded-lg">
            <h3 class="text-sm font-semibold text-gray-600 mb-4">Total Shortage Units</h3>
            <div class="space-y-3">
                <div>
                    <p class="text-xs text-gray-500 uppercase">Units to Restock</p>
                    <p class="text-2xl font-bold text-yellow-600">
                        {{ $inventories->sum(fn($i) => $i->min_quantity - $i->quantity) }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="card bg-white p-6 rounded-lg">
            <h3 class="text-sm font-semibold text-gray-600 mb-4">Estimated Cost</h3>
            <div class="space-y-3">
                <div>
                    <p class="text-xs text-gray-500 uppercase">Total to Restock All</p>
                    <p class="text-2xl font-bold text-green-600">
                        ₱{{ number_format($inventories->sum(fn($i) => ($i->min_quantity - $i->quantity) * $i->unit_price), 2) }}
                    </p>
                </div>
            </div>
        </div>
